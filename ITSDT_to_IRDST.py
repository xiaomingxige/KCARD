...
if __name__ == "__main__":
    ...
    print('target_train_annotation_path: ', target_train_annotation_path)
    #---------------------------------------------------------------------#
    ...
    num_workers         = Unfreeze_batch_size
    ...
    sam_checkpoint = None
    model_type = "vit_t"
    mobile_sam = sam_model_registry[model_type](checkpoint=sam_checkpoint).eval()
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(source_train_annotation_path, encoding='utf-8') as f:
        source_train_lines = f.readlines()
    with open(target_train_annotation_path, encoding='utf-8') as f:
    ...
    if local_rank == 0:
        show_config(
            ...
            source_num_train=source_num_train, target_num_train=target_num_train, target_num_val=target_num_val
        )
        ...
        if total_step <= wanted_step:
            ...
            wanted_epoch = wanted_step // (used_num_train // Unfreeze_batch_size) + 1
    ...
    if True:
        ...
        #------------------------------------#
        # if Freeze_Train:
        for param in model.mobile_sam.parameters():
        ...
        epoch_step      = used_num_train // batch_size
        epoch_step_val  = target_num_val // batch_size
        if epoch_step == 0 or epoch_step_val == 0:
            raise ValueError("数据集过小，无法继续进行训练，请扩充数据集。")
        if ema:
            ema.updates     = epoch_step * Init_Epoch
        
        source_train_dataset = source_seqDataset(source_train_annotation_path, input_shape[0], 5, 'train')
        target_train_dataset = target_seqDataset(target_train_annotation_path, input_shape[0], 5, 'train')
        target_val_dataset = target_seqDataset(target_val_annotation_path, input_shape[0], 5, 'val')
        ...
        if distributed:
            ...
            target_val_sampler     = torch.utils.data.distributed.DistributedSampler(target_val_dataset, shuffle=False,)
        ...
        source_gen = DataLoader(source_train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True,
                                    drop_last=True, collate_fn=source_dataset_collate, sampler=source_train_sampler)
        target_gen = DataLoader(target_train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True,
                                    drop_last=True, collate_fn=target_dataset_collate, sampler=target_train_sampler)
        target_gen_val = DataLoader(target_val_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True, 
                                    drop_last=True, collate_fn=target_dataset_collate, sampler=target_val_sampler)
        #----------------------#
        #   记录eval的map曲线
        #----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, class_names, num_classes, target_val_lines, 
                                         save_dir, Cuda, eval_flag=eval_flag, period=eval_period,
                                         source_train_annotation_path=source_train_annotation_path, val_source_train_dataset_length=target_val_dataset.length)
        else:
            eval_callback = None
        #---------------------------------------#
        #   开始模型训练
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):     
            ...
            fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, 
                        ...
                        save_dir, local_rank, file_name, input_shape)
            if distributed:
                dist.barrier()
        if local_rank == 0:
